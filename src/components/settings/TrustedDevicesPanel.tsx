...
/**
 * Trusted Devices workspace — mounted as a tab inside User Management
 * (/dashboard/settings/users?tab=devices). Renders no page header of its
 * own; the parent page owns that. Extracted from the former standalone
 * /dashboard/settings/devices route with behavior unchanged.
 */
        ...
        <div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                {(['all', 'pending', 'trusted', 'revoked'] as FilterStatus[]).map(s => (
                    <button
                        ...
                        className={`p-4 rounded-lg border transition-all text-left ${filterStatus === s
                            ? 'border-[#00458B]/40 bg-blue-50'
                            : 'border-[#DFE6EE] bg-white hover:bg-[#F7F9FB]'
                            }`}
                    >
                        <p className="text-[11px] uppercase tracking-widest text-[#5B6B7D] mb-1 capitalize">{s}</p>
                        <p className="text-2xl font-mono font-bold text-[#0E2B5C]">{counts[s]}</p>
                    </button>
                ))}
            </div>
            ...
            <div className="flex items-center gap-3 mb-5">
                <div className="relative flex-1">
                    <Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#5B6B7D]" />
                    <input
                        type="text"
                        placeholder="Search by device name, user, or platform…"
                        value={search}
                        onChange={e => setSearch(e.target.value)}
                        className="w-full pl-9 pr-4 py-2.5 rounded-lg bg-white border border-[#DFE6EE] text-sm text-[#0E2B5C] placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-[#00458B]/30 focus:border-[#00458B]"
                    />
                </div>
                <button
                    onClick={fetchDevices}
                    disabled={isLoading}
                    className="p-2.5 rounded-lg bg-white border border-[#DFE6EE] text-[#5B6B7D] hover:text-[#0E2B5C] hover:bg-[#F7F9FB] transition-all disabled:opacity-50"
                    title="Refresh"
                >
